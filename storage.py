...
class CertStore:
    ...
    def add_cert(self, cert):
        try:
            ...
            cur.execute(
                f"insert into cert( {CertStore._INSERT_DB_FIELDS_STR} ) values(?,?,?,?);",
    ...
    def get_cert(self, user_id, cert_id):
        try:
            cur = self.conn.cursor()
            cur.execute(
                "select id, user_id, cn, description, exp_date from cert where id = ? and user_id = ?;",
                (cert_id, user_id))
            rows = cur.fetchall()
        except Error as e:
            print(e)
    ...
    def find_by_cn(self, user_id, common_name):
        try:
            cur = self.conn.cursor()
            cur.execute(
                "select id, user_id, cn, description, exp_date from cert where user_id = ? and cn = ?;",
                (user_id, common_name))
            rows = cur.fetchall()
        except Error as e:
            print(e)
